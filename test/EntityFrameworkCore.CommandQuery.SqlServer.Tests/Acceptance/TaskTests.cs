...
namespace EntityFrameworkCore.CommandQuery.SqlServer.Tests.Acceptance
{
    public class TaskTests : DatabaseTestBase
    {
        ...
        public async Task FullTest()
        {
            ...
            var createCommand = new EntityCreateCommand<TaskCreateModel, TaskReadModel>(createModel, MockPrincipal.Default);
            var createResult = await mediator.Send(createCommand).ConfigureAwait(false);
            createResult.Should().NotBeNull();

            // Get Entity by Key
            var identifierQuery = new EntityIdentifierQuery<Guid, TaskReadModel>(createResult.Id, MockPrincipal.Default);
            ...
            var listQuery = new EntityPagedQuery<TaskReadModel>(entityQuery, MockPrincipal.Default);
            ...
            var patchCommand = new EntityPatchCommand<Guid, TaskReadModel>(createResult.Id, patchModel, MockPrincipal.Default);
            ...
            var updateCommand = new EntityUpdateCommand<Guid, TaskUpdateModel, TaskReadModel>(createResult.Id, updateModel, MockPrincipal.Default);
            var updateResult = await mediator.Send(updateCommand).ConfigureAwait(false);
            updateResult.Should().NotBeNull();
            updateResult.Title.Should().Be("Update Command");

            // Delete Entity
            var deleteCommand = new EntityDeleteCommand<Guid, TaskReadModel>(createResult.Id, MockPrincipal.Default);
        ...
        public async Task Upsert()
        {
            ...
            var upsertCommandNew = new EntityUpsertCommand<Guid, TaskUpdateModel, TaskReadModel>(key, updateModel, MockPrincipal.Default);
            var upsertResultNew = await mediator.Send(upsertCommandNew).ConfigureAwait(false);
            upsertResultNew.Should().NotBeNull();

            // Get Entity by Key
            var identifierQuery = new EntityIdentifierQuery<Guid, TaskReadModel>(key, MockPrincipal.Default);
            var identifierResult = await mediator.Send(identifierQuery).ConfigureAwait(false);
            identifierResult.Should().NotBeNull();
            ...
            var upsertCommandUpdate = new EntityUpsertCommand<Guid, TaskUpdateModel, TaskReadModel>(key, updateModel, MockPrincipal.Default);
        ...
        public async Task EntitySelectQuery()
        {
            ...
            var selectQuery = new EntitySelectQuery<TaskReadModel>(filter, MockPrincipal.Default);
